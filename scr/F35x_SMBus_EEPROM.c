...
// How To Test:
//
// 1) Download code to a 'F35x device that is connected to a 24LC02B serial
...
void SMBus_EEPROM_Init(void)
{
   ...
   while(!SDA)
   {
	...
	SMBus_PORT_Init();				   // Initialize Crossbar and GPIO
...
{
   //P0MDOUT = 0x00;                     // All P0 pins open-drain output(P0全部引脚开漏输出)：

   P0MDOUT = 0xC0;                     	//P1.7背光 P1.6按键,其余引脚全部开漏输出
